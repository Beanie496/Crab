use crate::{
    bits::{ east, north, pop_lsb, pop_next_square, south, to_square, west },
    board::Board,
...
impl Movegen {
    ...
    /// Calculates all attack permutations from `piece` on `square` and stores
    /// them in `attack_buffer`. Attacks generated by using a full mask of
    /// blockers and using the Carry-Rippler trick on the blocker mask for each
    /// subsequent attack.
    /// TODO: should this include the edge?
...
impl Movegen {
    /// Calculates the attack from `square` to the first encountered bit in
    /// `blockers` in the direction `direction`.
    /// TODO: Should this include the edge?
    fn ray_attack(
        square: Square,
        direction: Direction,
